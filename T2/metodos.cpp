...
double* newton(std::string funcao, double *x, int num_vars, double err) {
	...
	while (true) {
		...
		if ((get_norma2(gradiente, num_vars) < err) || (get_norma2(dif, num_vars) < err)) {
			free(d);
			free(gradiente);
			free_matriz_dinanica(hessiana, num_vars);
			free(dif);
			break;
		}
...
double* fletcher_reeves(std::string funcao, double* x, int num_vars, double erro) {
	...
	double* prox_direcao;
	double* prox_grad;
	double* grad = get_gradiente(funcao, x, num_vars, erro);
	double* direcao;
	...
	while (get_norma2(grad, num_vars) > erro) {

		direcao = get_const_mult_vetor(-1, grad, num_vars);

		for (int i = 0; i < num_vars; i++) {

			std::string lambda_func = transforma_string(funcao, x, direcao, 0);

			for (int j = 1; j < num_vars; j++ )
				lambda_func = transforma_string(lambda_func, x, direcao, j);

			exprtk::expression<double> func = prepara_func(x, num_vars, lambda_func);

			lambda = min_newton(func, x, num_vars, x[num_vars], DBL_MAX, erro);

			double* lambda_direcao = get_const_mult_vetor(lambda, direcao, num_vars);
			/* xk+1 = xk + lambda (nova aproximacao) */
			for (int i = 0; i < num_vars; i++)
				x[i] += lambda_direcao[i];

			printaVetor(x, num_vars);

			prox_grad = get_gradiente(funcao, x, num_vars, erro);
